...
def bgd_qr_index(request, token=None):
    ...
    # Xác định cuộc thi dùng cho QR (tạm thời lấy từ query ?ct=..., fallback cuộc thi đang bật)
    ct = None
    ...
    competitions = list(
        CuocThi.objects.order_by("-id").values("id", "tenCuocThi")
    )
